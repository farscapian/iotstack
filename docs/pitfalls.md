...
| esptool: "Unexpected chip ID in image... is not an ESP32-xx image. Use the force argument to flash anyway" during a serial bootstrap write | Historical: two `iotstack flash` invocations for different chip variants running at once both swapped one shared bootstrap build dir at the same time. Fixed structurally -- each variant now gets its own persistent build dir (`esphome.build_path` in `yamls/bootstrap.yaml`, keyed by `chip_variant`), so there is no shared dir left to race. `flash_lock_acquire()` (`scripts/flash-lock.sh`) still serializes `iotstack flash` invocations, now to protect the shared global partition-table artifact instead | If you still hit this on an old checkout, `git pull`; do not pass `--force` to esptool -- that flashes a wrong-chip image |
| OTBR snap never comes back after unplugging/replugging the ESP32-C6 radio (`otbr-agent` log: `Init() at hdlc_interface.cpp:154: No such file or directory`, then `Start request repeated too quickly`) | systemd start limit hit while the radio was absent; unit stays `failed` and nothing restarts it | Fixed: `iotstack otbr snap` installs `20-retry-forever.conf` drop-in (`StartLimitIntervalSec=0`, `RestartSec=5`). Re-run `iotstack otbr snap`, or one-off: `sudo systemctl reset-failed snap.openthread-border-router.otbr-agent && sudo snap start openthread-border-router` |
