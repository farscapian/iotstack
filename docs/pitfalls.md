...
| esptool: "Unexpected chip ID in image... is not an ESP32-xx image. Use the force argument to flash anyway" during a serial bootstrap write | Two `iotstack flash` invocations for different chip variants running at once both swapped the shared bootstrap build cache (`iotstack_bootstrap_swap_build_cache`) at the same time | Fixed: `flash_lock_acquire()` (`scripts/flash-lock.sh`) serializes all `iotstack flash` invocations with a `flock`; a second invocation now waits instead of racing. If you still hit this, `git pull` for the fix; do not pass `--force` to esptool -- that flashes a wrong-chip image |
