...
CLAUDE_PARENT="${IOTSTACK_CLAUDE_WORKTREES:-${HOME}/.claude/worktrees/mini-projects-iotstack}"
...
# OSC 8 terminal hyperlink (VS Code, iTerm2, etc.)
print_hyperlink() {
  local url="$1"
  local label="${2:-$url}"
...
Examples:
  ...
  /home/derek/Sync/mini_projects/iotstack/scripts/init_claude_session.sh

  init_claude_session.sh ~/.claude/worktrees/mini-projects-iotstack/<session-id>
EOF
}
...
if [[ "$(readlink -f "$REPO_ROOT")" == "$(readlink -f "$SYNC_REPO")" ]]; then
  ...
  warn "Init is intended for ~/.claude/worktrees/mini-projects-iotstack/<session-id>/"
...
cat <<'EOF'
================================================================================
Using Claude Code agents efficiently (iotstack)
================================================================================
