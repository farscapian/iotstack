...
```bash
git clone https://github.com/farscapian/otbrstack-commissioner-stacks.git
cd otbrstack-commissioner-stacks
source ./otbrstack.sh
```
...
```bash
source /path/to/otbrstack-commissioner-stacks/otbrstack.sh
```

### 2. Create your env file

Env files live in `~/.otbrstack/env/`. Copy an example and fill in your values:

```bash
# Name it after your machine (hostname-specific, takes priority):
cp .env.example ~/.otbrstack/env/$(hostname).env
nano ~/.otbrstack/env/$(hostname).env

# Or use the generic fallback name:
cp .env.example ~/.otbrstack/env/.env
```
...
hostname (default: `otbrstack-raspi4`) so you can reach it by name after boot.
If one doesn't exist, the script will offer to create it. You can also add
it manually:

```
# ~/.ssh/config
Host otbrstack-raspi4
    User ubuntu
...
otbrstack flash --hostname=otbrstack-kitchen /dev/sdX
```
...
otbrstack logs -f otbrstack-raspi4

# Or SSH in and check the firstboot log directly
ssh otbrstack-raspi4
sudo tail -f /var/log/otbrstack-firstboot.log
```
...
otbrstack logs otbrstack-raspi4

# Reboot the device
otbrstack restart otbrstack-raspi4

# Graceful shutdown
otbrstack shutdown otbrstack-raspi4
```
...
```bash
otbrstack vm x64             # VM (default), instance name: otbrstack-test-x64
otbrstack vm x64 --container # system container (faster), name: otbrstack-test-ct
...
```bash
incus delete otbrstack-test-x64 --force
```

### Running the test suite

```bash
sudo ./tests/test_otbrstack_vm.sh             # full test, x64 VM
sudo ./tests/test_otbrstack_vm.sh --no-peer-test  # skip neighbor exchange (T6)
```
...
Installs Docker CE, pulls `openthread/otbrstack`, writes a stable udev symlink for
...
| `OTBR_HOSTNAME` | flash | Device hostname (default: `otbrstack-raspi4`) |
...
otbrstack-docker-setup.sh  # Bare-metal Docker provisioner (called by otbrstack docker)
scripts/
