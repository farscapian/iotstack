...
build_and_flash_rcp() {
    local port="$1"
    local cached_bin="${OTBR_HOME}/cache/esp32/rcp/esp_ot_rcp.bin"
    ...
    if command -v idf.py &>/dev/null; then
        ...
        _idf_path="${OTBR_HOME}/cache/esp-idf"
        if [[ ! -d "$_idf_path" ]]; then
            info "Cloning ESP-IDF into ${OTBR_HOME}/cache/esp-idf ..."
    ...
    # ------------------------------------------------------------------
    cat > "${ot_rcp_dir}/sdkconfig.defaults.otbr" <<'SDKEOF'
...
# Ensure the target project exists. Run incus with INCUS_PROJECT unset so the
# show/create commands run against the default project (not self-referential).
_incus_project_name="$INCUS_PROJECT"
if ! INCUS_PROJECT="" incus project show "$_incus_project_name" &>/dev/null; then
    info "Creating Incus project '${_incus_project_name}' ..."
    INCUS_PROJECT="" incus project create "$_incus_project_name" \
        -c features.images=false \
        -c features.profiles=false
    info "Project '${_incus_project_name}' created (shares images + profiles with default)."
fi
...
SIM_RCP_DIR="${OTBR_HOME}/cache/ot-rcp-sim"
SIM_RCP_BIN_PATH="${SIM_RCP_DIR}/ot-rcp"
_OT_SRC="${OTBR_HOME}/cache/openthread"
...
SNAP_CACHE="${OTBR_HOME}/cache/snap"
...
# shellcheck disable=SC2016  # single quotes intentional: envsubst variable list
...
_ARTIFACT_DIR="${OTBR_HOME}/artifacts/${_ARTIFACT_SUBDIR}/${INSTANCE_NAME}/${_TS}"
...
while (( SECONDS < DEADLINE )); do
    if incus exec "$INST" -- true 2>/dev/null; then break; else sleep 2; fi
done
...
if timeout "$BOOT_TIMEOUT" \
    ...
    rc=$?
    if [[ $rc -eq 124 ]]; then
        die "cloud-init timed out after ${BOOT_TIMEOUT}s."
    else
...
# shellcheck disable=SC2016  # single quotes intentional: remote shell script
REMOTE_CMD='
