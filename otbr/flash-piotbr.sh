...
# 1.2.5 Parse flags (env is loaded by otbr before invoking this script)
_HOSTNAME_FLAG=""
_FORCE_FLASH=0
_SKIP_CONFIRM=0
_POSARGS=()
...
if [[ "$CLOUD_INIT_ONLY" -eq 0 ]]; then
    ...
    if [[ -f "$IMAGE_IMG" ]]; then
        _min_bytes=$(stat -c %s "$IMAGE_IMG")
    else
        _min_bytes=7516192768  # 7 GiB floor when image not yet downloaded
    fi
    if [[ "$_dev_bytes" -lt "$_min_bytes" ]]; then
        _dev_h=$(numfmt --to=iec-i --suffix=B "$_dev_bytes" 2>/dev/null || echo "${_dev_bytes} bytes")
        _min_h=$(numfmt --to=iec-i --suffix=B "$_min_bytes" 2>/dev/null || echo "${_min_bytes} bytes")
        die "$TARGET_DEV is too small to flash.\n  Device:   ${_dev_h}\n  Required: ${_min_h}\n  Use a larger SD card."
    fi
    unset _dev_bytes _min_bytes _dev_h _min_h
...
# ---------------------------------------------------------------------------
# Build optional Home Assistant MQTT Discovery cloud-init section.
# All three variables are empty when MQTT_BROKER is unset.
# ---------------------------------------------------------------------------
_MQTT_PACKAGES_YAML=""
_MQTT_WRITE_FILES_YAML=""
_MQTT_ENABLE_CMD=""
if [[ -n "$MQTT_BROKER" ]]; then
    info "MQTT Discovery enabled: ${MQTT_BROKER}:${MQTT_PORT}"
    _MQTT_PACKAGES_YAML="  - python3-paho-mqtt"
    _MQTT_ENABLE_CMD="  - systemctl enable otbr-ha-mqtt.service"
    _MQTT_WRITE_FILES_YAML=$(cat <<MQTTEOF

  # 9.1.12 Home Assistant MQTT Discovery
  - path: /etc/otbr/mqtt.env
    owner: root:root
    permissions: '0600'
    content: |
      MQTT_BROKER=${MQTT_BROKER}
  ...
  - path: /usr/local/bin/otbr-ha-mqtt.py
    owner: root:root
    permissions: '0755'
    content: |
      #!/usr/bin/env python3
      """HA MQTT Discovery: restart/shutdown buttons, OTBR agent + Thread role sensors."""
      import json, logging, os, socket, subprocess, time
      import paho.mqtt.client as mqtt

      logging.basicConfig(level=logging.INFO,
                          format='%(asctime)s otbr-ha-mqtt %(levelname)s: %(message)s')
      ...
      DEVICE = {
          'identifiers': ['otbr_' + NODE],
          'name':         'OTBR ' + NODE,
          'model':        'Raspberry Pi OTBR',
          'manufacturer': 'farscapian/otbr-commissioner-stacks',
      }
      AVAIL = 'otbr/' + NODE + '/availability'

      def _disc(comp, obj):
          return 'homeassistant/' + comp + '/otbr_' + NODE + '/' + obj + '/config'
      def _cmd(obj):   return 'otbr/' + NODE + '/' + obj + '/cmd'
      def _state(obj): return 'otbr/' + NODE + '/' + obj + '/state'

      DISCOVERY = [
          (_disc('button', 'restart'), {
              'name': 'Restart', 'unique_id': 'otbr_' + NODE + '_restart',
              'command_topic': _cmd('restart'), 'availability_topic': AVAIL,
              'device': DEVICE, 'icon': 'mdi:restart',
          }),
          (_disc('button', 'shutdown'), {
              'name': 'Shutdown', 'unique_id': 'otbr_' + NODE + '_shutdown',
              'command_topic': _cmd('shutdown'), 'availability_topic': AVAIL,
              'device': DEVICE, 'icon': 'mdi:power',
          }),
          (_disc('binary_sensor', 'agent'), {
              'name': 'OTBR Agent', 'unique_id': 'otbr_' + NODE + '_agent',
              'state_topic': _state('agent'), 'availability_topic': AVAIL,
              'device': DEVICE, 'device_class': 'running',
              'payload_on': 'ON', 'payload_off': 'OFF',
          }),
          (_disc('sensor', 'thread_role'), {
              'name': 'Thread Role', 'unique_id': 'otbr_' + NODE + '_thread_role',
              'state_topic': _state('thread_role'), 'availability_topic': AVAIL,
              'device': DEVICE, 'icon': 'mdi:thread',
          }),
          (_disc('sensor', 'uptime'), {
              'name': 'Uptime', 'unique_id': 'otbr_' + NODE + '_uptime',
              'state_topic': _state('uptime'), 'availability_topic': AVAIL,
              'device': DEVICE, 'unit_of_measurement': 's',
              'device_class': 'duration', 'icon': 'mdi:clock-outline',
          }),
      ]
      ...
      def _get_states():
          active = _run('systemctl', 'is-active',
                        'snap.openthread-border-router.otbr-agent')
          role   = _run('snap', 'run',
                        'openthread-border-router.ot-ctl', 'state')
          try:
              uptime = str(int(float(open('/proc/uptime').read().split()[0])))
          except Exception:
              uptime = ''
          return {
              'agent':       'ON' if active == 'active' else 'OFF',
              'thread_role': role.splitlines()[0] if role else 'unknown',
              'uptime':      uptime,
          }

      def _publish_states(c):
          for k, v in _get_states().items():
              c.publish(_state(k), v, retain=True)

      def _on_connect(c, _ud, _fl, rc):
          if rc != 0:
              log.error('Connect failed rc=%d', rc); return
          ...
          c.subscribe(_cmd('restart'))
          c.subscribe(_cmd('shutdown'))
          _publish_states(c)

      def _on_message(c, _ud, msg):
          log.info('cmd: %s', msg.topic)
          if   msg.topic == _cmd('restart'):
              subprocess.Popen(['/sbin/reboot'])
          elif msg.topic == _cmd('shutdown'):
              subprocess.Popen(['/sbin/shutdown', '-h', 'now'])

      c = mqtt.Client(client_id='otbr-' + NODE, clean_session=True)
      if USER:
      ...
      while True:
          try:
              ...
              log.error('Error: %s - reconnecting in 30s', exc)
              try:
  ...
  - path: /etc/systemd/system/otbr-ha-mqtt.service
    owner: root:root
    permissions: '0644'
    content: |
      [Unit]
      Description=Home Assistant MQTT Discovery agent
      After=network-online.target snap.openthread-border-router.otbr-agent.service
      Wants=network-online.target

      [Service]
      Type=simple
      EnvironmentFile=/etc/otbr/mqtt.env
      ExecStart=/usr/bin/python3 /usr/local/bin/otbr-ha-mqtt.py
      Restart=on-failure
      RestartSec=30
...
AAEOF
  ...
  - path: /usr/local/bin/otbr-gpio-button.py
    ...
    content: |
      #!/usr/bin/env python3
      """GPIO button handler: 1 click = shutdown, 2 clicks = reboot.
      Button wiring: BCM GPIO17 (physical pin 11) to GND (physical pin 14).
      """
      import os, signal, subprocess, threading
      os.environ.setdefault('GPIOZERO_PIN_FACTORY', 'lgpio')
      from gpiozero import Button

      BUTTON_PIN = 17            # BCM GPIO17 = physical board pin 11
      DOUBLE_CLICK_WINDOW = 0.4  # seconds within which a 2nd press counts
      ...
      def _pressed():
          global _timer, _count
          _count += 1
          n = _count
          if _timer:
              _timer.cancel()
          _timer = threading.Timer(DOUBLE_CLICK_WINDOW, _fire, [n])
          _timer.start()

      Button(BUTTON_PIN, pull_up=True, bounce_time=0.05).when_pressed = _pressed
      signal.pause()

  - path: /etc/systemd/system/otbr-gpio-button.service
    owner: root:root
    permissions: '0644'
    content: |
      [Unit]
      Description=GPIO button handler (1 click: shutdown, 2 clicks: reboot)
      After=multi-user.target

      [Service]
      Type=simple
      ExecStart=/usr/bin/python3 /usr/local/bin/otbr-gpio-button.py
      Restart=on-failure
      RestartSec=5
      StandardOutput=journal
      StandardError=journal
...
runcmd:
  ...
  - systemctl enable otbr-gpio-button.service
${_MQTT_ENABLE_CMD}
...
CI_ARTIFACT_DIR="${OTBR_HOME}/artifacts/rpi/${OTBR_HOSTNAME}"
...
UDEV
        ...
        if [[ "$CLOUD_INIT_ONLY" -eq 0 ]]; then
            ...
            # ------------------------------------------------------------------
            _IDF_CACHE="${OTBR_HOME}/cache/esp-idf"
...
if [[ -x /opt/esp-idf/install.sh ]]; then
    ...
    if [[ "${_do_rcp_build}" == "1" ]]; then
        ...
        cat > "\${_rcp_dir}/sdkconfig.defaults.otbr" <<'SDKEOF'
...
CHROOTSCRIPT
            ...
            _IDF_TOOLS_CACHE="${OTBR_HOME}/cache/esp-idf-tools-arm64"
            ...
            if [[ "$_do_rcp_build" -eq 1 && -f "$_built_bin" ]]; then
                ...
                sudo cp "$_built_bin" "${OTBR_HOME}/cache/esp32/rcp/esp_ot_rcp.bin"
            ...
            # ------------------------------------------------------------------
            _SNAP_ARM64="${OTBR_HOME}/cache/snap-arm64"
            mkdir -p "$_SNAP_ARM64"
            ...
            _cache_snap() {
                ...
                if compgen -G "${_SNAP_ARM64}/${_sname}_*.snap" > /dev/null 2>&1; then
                    info "${_sname} arm64 snap already cached: $(basename "$(find "${_SNAP_ARM64}" -maxdepth 1 -name "${_sname}_*.snap" | head -1)")"
                    return 0
                fi
...
" "$_channel")
            ...
            if [[ -f "${OTBR_HOME}/cache/esp32/rcp/esp_ot_rcp.bin" ]]; then
                ...
                sudo cp "${OTBR_HOME}/cache/esp32/rcp/esp_ot_rcp.bin" \
...
info " Logs: otbr logs -f ${OTBR_HOSTNAME}"
